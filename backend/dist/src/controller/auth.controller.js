...
const constant_1 = __importDefault(require("../contants/constant"));
const signup = async (req, res) => {
    try {
        ...
        const profile_url = (0, constant_1.default)(gender, userName);
        // create user
...
const login = async (req, res) => {
    try {
        ...
        const userExists = await prisma_1.default.user.findUnique({ where: { userName } });
        if (!userExists) {
        ...
        (0, generateToken_1.generateToken)(userExists.id, res);
        res.status(200).json({
            id: userExists.id,
